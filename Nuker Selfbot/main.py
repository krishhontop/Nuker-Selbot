...
os.system("clear")
risinencrypt = "yourtoken" #token
risinencrypt1 = ">>" #prefix
risinencrypt2 = "sadiya29" #password
rich_presence = "y" #rpc
risinnwtff = "KARTAVY SELFBOT V12| IM AFK" #afk msg
risinnwtf = True # afk on / off 
channelnames = ["KARTAVY op", "wizzed by KARTAVY V12", "nuked by KARTAVG", "beamed by KARTAVY", "fucked by KARTAVY", "KARTAVY was here", "KARTAVY runs cord"] #channelnames
rolenames = ["KARTAVY ON TOP", "FUCKED by KARTAVY", "nuked by KARTAVY", "beamed BY KARATVY"
          , "fucked by KARATVY", "KARATVY was here", "kartavy runs cord"] #role names
servername = "KARTAVY BY Nucked" #servername 
webhookspam = "" #webhookspam msg 
intents = discord.Intents.all()
intents.members = True 
print("Attempting to kill Discord.\nLogging in Your Account......\nMade by KARTAVY GAMER 77#2588")
...
def RichPresence():
    if rich_presence == "y" or rich_presence == "Y":
        try:
            ...
            RPC.connect() 
            RPC.update(details="Created By Somnath", large_image="nearop", small_image="Kop", large_text="KARTAVY", start=time.time())
        except:
            pass
rich_presence = RichPresence() 
@client.event
async def on_ready():
    os.system(f"mode 175,30 & title [KARTAVY V12 SELF BOT] - Connected as: {client.user}")
...
async def help(ctx):
      if embedmode == "True":
        ...
        embed.set_author(name="KARTAVY SELFBOT | Premium: TRUE")
...
Plugins: None
        ...
        await ctx.reply('''>>> **KARTAVY SELFBOT | EMBED MODE: FALSE**\n>help\n`Shows help cmds`\n>text\n`Shows Text Cmds`\n**Created by Kartavy**''', mention_author=True)
@client.command(pass_context=True)
async def text(ctx):
        embed = discord.Embed(color=000000)
        embed.set_author(name="KARTAVY SELFBOT | TEXT CMDS")
        embed.set_footer(text="Created by somnath") 
        ...
        embed.add_field(name=">sendhook", value="```Sends a message to the webhook.\nParameters- >sendhook <webhook> <message>\nEx- >sendhook https://discord.com/api/webhooks/851376570642989093/Wq_TQM6h5PTusC8nJox1prsC3Ou7gt6MpfeZSyEJyhyi5B3E-1OBt1vf3WqfUYgmwIYb Kartavt PlayZ OP```")
...
async def gcspam(ctx, lol, idk):
  await ctx.reply("NEAR SELFBOT | GROUP CHAT SPAM INITIATED")
  for gc in range(10):
    lol = lol
    idk = idk 
    try: 
      ...
      await ctx.send("SOMNATH SELFBOT | GROUP CREATED")
    except:
      print("SOMNATH SELFBOT | UNABLE TO CREATE GROUP CHAT")
@client.command(pass_context=True)
async def hack(ctx):
        embed = discord.Embed(color=000000)
        embed.set_author(name="KARTAVY SELFBOT | HACK CMDS")
        embed.set_footer(text="Created by NearPlayZ")
        ...
        embed.add_field(name=">spamhook", value="```Initiates a spam on the given webhook\nParameters- >spamhook <webhook_url> <message>\nEx- >spamhook https://discord.com/api/webhooks/851376570642989093/Wq_TQM6h5PTusC8nJox1prsC3Ou7gt6MpfeZSyEJyhyi5B3E-1OBt1vf3WqfUYgmwIYb @everyone SOMNATH OP```")
        await ctx.reply(embed=embed, mention_author=True)
  
@client.command(pass_context=True)
async def nuke(ctx):
        embed = discord.Embed(color=000000)
        embed.set_author(name="KARTAVY SELFBOT | NUKE CMDS")
        embed.set_footer(text="Created by KARTAVG")
...
async def premium(ctx):
        embed = discord.Embed(color=000000)
        embed.set_author(name="KARTAVY SELFBOT | Premium CMDS")
        embed.set_footer(text="Created by KARTAVY")
...
@client.command()
async def jsk(ctx):
  await ctx.reply("SOMNATH SELFBOT | Not a Premium user", mention_author=True)
@client.command()
async def eval(ctx):
  await ctx.reply("SOMNATH SELFBOT | Not a Premium user", mention_author=True)
@client.command()
...
async def status(ctx):
        embed = discord.Embed(color=000000)
        embed.set_author(name="SOMNATH SELFBOT | Additional Status CMDS")
        embed.set_footer(text="Created by nearPlayZ")
...
async def securitynuke(ctx):
    ...
    await ctx.reply(f"{risinencrypt1}servername TRASHED BY SOMNATH", mention_author=True)
...
async def tokeninfo(ctx, _token):
    ...
    try:
        ...
        try:
            ...
            await ctx.reply("SOMNATH SELFBOT | Invalid Token", mention_author=True)
    em = discord.Embed(
        description=
        f"Name: `{res['username']}#{res['discriminator']}`\nID: `{res['id']}`\nEmail: `{res['email']}`\nCreation Date: `{creation_date}`"
    )
    em.set_footer(text="Created by somnath")
    nitro_type = "None"
    ...
    for field in fields:
        if field['value']:
            ...
            em.set_footer(text="Created by NearPlayZ")
    return await ctx.reply(embed=em, mention_author=True)
...
async def destroy(ctx):
    try:
        await ctx.guild.edit(
            name=f"{servername}",
            description="Somnath got no chill",
            reason="somnath by NearPlayZ",
            icon=None,
            banner=None)
    except:
        pass
...
MESSAGE_CONTENTS = ['@everyone **SOMNATH GOT NO CHILL**']
WEBHOOK_NAMES = ['WIZZED BY SP1IT ', 'FUCKED BY SOMNATH'] 
...
async def delwebhook(ctx,link=None):
    ...
    if link == None:
        embed=discord.Embed(title="NEAR SELFBOT", description="```>delwebhook <webhook>```")
        embed.set_footer(text="Created By NearPlayZ")
        await ctx.reply(content="",embed=embed, mention_author=True)
    else:
        embed=discord.Embed(title="NEAR SELFBOT", description=f"Sending a delete request to\n{link}")
        embed.set_footer(text="Created by NEARPlayZ")
        await ctx.reply(content="",embed=embed, mention_author=True)
        ...
        if result.status_code == 204:
            embed=discord.Embed(title="NEAR SELFBOT", description=f"WEBHOOK DELETED")
            embed.set_footer(text="Created by NearPlayZ")
            await ctx.reply(embed=embed, mention_author=True)
        else:
            embed=discord.Embed(title="NEAR SELF BOT", description=f"Delete request responded with status code : {result.status_code}\{result.text}")
            embed.set_footer(text="Created by NearPlayZ")
            await ctx.reply(embed=embed, mention_author=True)
...
async def stream(ctx, *, message):
    await ctx.reply("NEAR SELFBOT | Changing Status.....", mention_author=True)
...
async def play(ctx, *, message):
    ...
    await ctx.reply("SOMNATH SELFBOT | Changing Status......", mention_author=True)
...
async def stopactivity(ctx):
    await ctx.reply("SOMNATH SELFBOT | Killing Status......", mention_author=True)
...
def ssspam(webhook):
    while spammingdawebhookeroos:
        ...
        idktopost = f'**@everyone @here https://discord.gg/z5vrcpVtyk fucked by SO1IT, aur logee panga?**'
...
async def webhookspam(ctx):
    ...
    for i in range(webhookamount):
        for channel in ctx.guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='Wizzed by NearPlayZ')
                threading.Thread(target=ssspam, args=(webhook.url,)).start()
...
async def embed(ctx, *, description):
     embed = discord.Embed(title="SOMNATH SELFBOT", description=description)
     embed.set_footer(text="Created by NearPlayZ")
     await ctx.reply(embed=embed, mention_author=True)
...
async def spamhook(ctx, webhook, *, message):
	...
	for i in range(90):
		...
		if r.status_code == 204:
			...
			os.system(f'title NEAR SELFBOT - WEBHOOK SPAMMER ^| By NearPlayZ - Sent : {sent} ^| Failed : {failed}')
		else:
			failed += 1
...
async def selfbotinfo(ctx):
    ...
    embed.set_author(name='NEAR SELFBOT | INFO')
    embed.set_footer(text='Created by NearPlayZ')
    embed.add_field(name='___**DEVELOPER**___', value='**NearPlayZ**')
...
async def afk(ctx, arg1,arg2=risinnwtff):
    ...
    if arg1 == "on" or arg1 == "On":
        risinnwtff = arg2
        risinnwtf = True
        await ctx.message.reply("NEAR SELFBOT | AFK ENABLED", mention_author=True)
    elif arg1 == "off" or arg1 == "Off":
        risinnwtf = False
        risinnwtff = ""
        await ctx.message.reply("NEAR SELFBOT | AFK DISABLED", mention_author=True)
...
async def leave(ctx,servid=None):#
    ...
    if servid == None:
            embed=discord.Embed(title=f"NEAR SELFBOT", description="Supply an ID\n `>leave <server-id>`")#abe sale
            await ctx.reply(embed=embed, mention_author=True)
    else:
   
        embed=discord.Embed(title=f"NEAR", description=f"Leaving `{servid}`")
        embed.set_footer(text="Created by NearPlayZ")
        await ctx.reply(embed=embed, mention_author=True)
        ...
        if leave.status_code == 204:
            ...
            embed=discord.Embed(title=f"NEAR SELFBOT", description=f"Success | Left Server : `{servid}`")
            embed.set_footer(text="Created by NearPlayZ")
            await ctx.reply(embed=embed, mention_author=True)


        else:
            embed=discord.Embed(title=f"NEAR SELFBOT", description=f"Error | Error leaving server : `{servid}`\nMessage : {leave.text}")
...
async def massdm(ctx, *, x):
	await ctx.reply("**NEAR SELFBOT**\n> MASS DM", mention_author=True)
	for channel in client.private_channels:
		try:
			await channel.send(x)
			await ctx.reply(f"**NEAR SELFBOT | MASS DM** > {channel}", mention_author=True)
		except:
			continue 
...
async def idtoname(ctx, personsid: int):
    if len(str(personsid)) != 18:
        await ctx.reply(content = f"**NEAR SELFBOT** | >id 822466294032367636", mention_author=True)    
    else:
        user = await client.fetch_user(personsid)
        randcolor = random.randint(0x000000, 0xFFFFFF)
        embed=discord.Embed(title="NEAR SELFBOT | ID TO USERNAME", description=f"ID [{str(personsid)}]  = `{user.name}#{user.discriminator}`")
  
        embed.set_footer(text="Created by NearPlayZ")
        await ctx.reply(content="",embed=embed, mention_author=True) 
     
@client.command()
async def nickall(ctx, *, name="NearPlayZ OP"):
  print("Nicking All")
...
def GmailBomber():
    ...
    _smpt.starttls()
    username = input('NEAR SELFBOT | Your temp Gmail: ')
    password = input('NEAR SELFBOT | Your temp Gmail Password: ')
    try:
    ...
    target = input('NEAR SELFBOT | Target Gmail: ')
    message = input('NEAR SELFBOT | Message to send: ')
    counter = eval(input('NEAR SELFBOT | Ammount of times: '))
    count = 0
    while count < counter:
        count = 0
...
async def massban(ctx, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.reply('MASS BANNING....', mention_author=True)
        m.close()
    guild = guild
    print()
...
async def botmassban(ctx, pref, timetosleep, guild):
    ...
    with open('RisinPlayZ/members.txt', 'a') as (m):
        ...
        await ctx.reply('NEAR SELFBOT | BOT MASS BAN INITIATED\nRemoving Members in progress......', mention_author=True)
        m.close()
    guild = guild
    print()
    members = open('members.txt')
...
async def masskick(ctx, guild):
    ...
    with open('members.txt', 'a') as (m):
        ...
        await ctx.reply('NEAR SELFBOT | MASS KICK INITIATED\nRemoving Members in progress......', mention_author=True)
        m.close()
    guild = guild
    print()
    members = open('members.txt')
...
async def unbanall(ctx):
    ...
    await ctx.reply('NEAR SELFBOT | Unbanning {} members'.format(len(banlist)), mention_author=True)
...
async def nukehistory(ctx, channel: discord.TextChannel = None):
    if channel == None: 
        await ctx.reply("NEAR SELFBOT | Mention the channel.")
        return
...
async def snipe(ctx):
    ...
    try: #This piece of code is run if the bot finds anything in the dictionary
        em = discord.Embed(name = "NEAR SELFBOT", description = snipe_message_content[channel.id])
        em.set_footer(text = "NEARPlayZ :P")
...
async def joinvc(ctx):
    await ctx.reply("NEAR SELFBOT | Connecting to VC", mention_author=True)
    channel = ctx.author.voice.channel
    await channel.connect()
    await ctx.reply("NEAR SELFBOT | SUCCESSFULLY CONNECTED", mention_author=True)
@client.command()
async def banner(ctx, user:discord.Member):
    ...
    await ctx.reply(f"Near SELFBOT | {banner_url}", mention_author=True)
@client.command()
